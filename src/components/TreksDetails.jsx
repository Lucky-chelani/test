...
import { 
  ...
  FiInfo, FiUsers, FiShield, FiLoader
} from 'react-icons/fi';
...
import { sampleTrekData } from "../utils/sampleTrekData";
...
const HeroImage = styled.div`
  width: 100%;
  height: 100%;
  background-image: url(${props => getValidImageUrl(props.image)});
...
const TrekDetails = () => {
  ...
  // Helper function to get a valid image URL or use a placeholder
  const getValidImageUrl = (imageUrl) => {
    if (!imageUrl || imageUrl === '') {
      return 'https://via.placeholder.com/1200x800?text=Trek+Image+Coming+Soon';
    }
    return imageUrl;
  };
    // Fetch trek data from Firebase
  useEffect(() => {
    const fetchTrekData = async () => {
      try {
        setLoading(true);
        
        // First try to find by the slug/id in the id field
        const treksCollection = collection(db, "treks");
        const q = query(treksCollection, where("id", "==", id));
        const querySnapshot = await getDocs(q);
        
        let trekData = null;
        
        if (!querySnapshot.empty) {
          // Found the trek by id/slug
          trekData = {
            id: querySnapshot.docs[0].id,
            ...querySnapshot.docs[0].data()
          };
        } else {
          // Try to find by Firestore document ID
          const trekDoc = doc(db, "treks", id);
          const trekSnapshot = await getDoc(trekDoc);
        ...
        setTrek({
          id: trekData.id,
          title: trekData.title || "Trek Details",
          image: getValidImageUrl(trekData.image),
          country: "India",
    ...
    // If there's an ID, try to fetch the data from Firebase
    if (id) {
      fetchTrekData();
    }
  }, [id]);
  useEffect(() => {
    // In the future, fetch real trek data here
    // For now we're using the sample data
    setTrek(sampleTrekData);
    setLoading(false);
  }, [id]);
  
  // Calculate average rating
  const calculateAverage = () => {
    if (!trek || !trek.ratingBreakdown) return "0.0";
